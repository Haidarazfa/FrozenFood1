const products = {
    ...
    "Muantap": [
        { name: "Muantap Frankfurer Mini 500g", price: 26000 }
    ],
    "Ngetop": [
        { name: "Ngetop Kornet 450g", price: 18000 }
    ],
    "Palmia": [
        { name: "Palmia", price: 7000 }
    ],
    "Nala": [
        { name: "Pempek Nala", price: 17000 }
    ],
    "Salam": [
    ...
    "So Eco": [
        { name: "So Eco Nugget 500g", price: 20000 }
    ],
    "So Good": [
...
function showModal(brand) {
    ...
    if (!products[brand]) {
        showNotification('⚠️ Produk belum tersedia untuk merek ini');
        return;
    }
    ...
    products[brand].forEach(product => {
        ...
        row.innerHTML = `
            ...
            <td><button onclick="addToCart('${brand}', '${product.name.replace(/'/g, "\\'")}')">➕ Tambah</button></td>
        `;
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.brand-grid button').forEach(button => {
        ...
        button.onclick = () => showModal(brand);
    });
